...
import { generateVideoPrompt, generateVideoPromptFromTextAndImage } from '../services/geminiService';
...
const guideSteps = [
    {
        ...
        description: "Provide an image to set the scene for your video prompt."
    },
    {
        ...
        title: "Enter a Theme",
        description: "Add keywords or a theme to guide the video's narrative and action."
    },
    {
        icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>,
        title: "Generate Cinematic Prompt",
        description: "The AI will create a detailed prompt with camera angles and scene details."
    },
];
...
const VideoPromptGenerator: React.FC = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            let generatedPrompt;
            if (imageFile) {
                generatedPrompt = await generateVideoPromptFromTextAndImage(keywords, imageFile);
            } else {
                generatedPrompt = await generateVideoPrompt(keywords);
            }
    ...
    return (
        <div className="max-w-7xl mx-auto">
            <ToolHeader toolKey={ToolKey.VideoPromptGenerator} />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="md:sticky md:top-24 h-fit">
                            <div className="glass-card rounded-lg p-6 space-y-4">
                                ...
                                <div>
                                     <h3 className="text-lg font-medium text-slate-700 mb-2">Keywords / Theme</h3>
                                    <div className="relative">
                                        <textarea
                                            value={keywords}
                                            onChange={(e) => setKeywords(e.target.value)}
                                            placeholder="e.g., epic space battle, slow motion"
                                ...
                                <button
                                    ...
                                    disabled={loading}
                                    title="Generate a cinematic video prompt"
                        ...
                        <div>
                            {loading ? (
                                <ProcessingView
                                    title="Generating Cinematic Prompt..."
                                    messages={[ "Envisioning the scene...", "Planning camera movements...", "Defining lighting and color...", "Writing the prompt..." ]}
                                    previewUrl={preview ?? undefined}
                                />
                            ) : (
                                prompt && <ResultCard title="Generated Video Prompt" content={prompt} />
                            )}
                        </div>
                    </div>
                </div>
